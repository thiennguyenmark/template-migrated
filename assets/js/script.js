    ...
    $('.contact-form').unbind('submit').bind('submit', function (e) {
      ...
      $.post(form.data('action'), {data: JSON.stringify(paramObj)}, function(res){
        if (res.status == 200) {
          ...
          alert('Liên hệ thành công, chúng tôi liên hệ lại quý khách trong khoảng thời gian sớm nhất.');
        } else {
    ...
    $('#test-drive-registration-form').unbind('submit').bind('submit', function (e) {
      ...
      $.post(form.data('action'), {data: JSON.stringify(paramObj)}, function(res){
        if (res.status == 200) {
          ...
          alert('Đăng ký thành công, chúng tôi liên hệ lại quý khách trong khoảng thời gian sớm nhất.');
        } else {
          alert('Có lỗi xảy ra, vui lòng thử lại.');
        }
      });
    ...
    $('#equated-monthly-installment-form').unbind('submit').bind('submit', function (e) {
      ...
      $.post(form.data('action'), {data: JSON.stringify(paramObj)}, function(res){
        if (res.status == 200) {
          ...
          form.closest('.modal').hide();
          alert('Đã gửi yêu cầu ước tính trả góp thành công, chúng tôi liên hệ lại quý khách trong khoảng thời gian sớm nhất.');
        } else {
          alert('Có lỗi xảy ra, vui lòng thử lại.');
        }
      });
    ...
    $('#request-for-quotation-form').unbind('submit').bind('submit', function (e) {
      ...
      $.post(form.data('action'), {data: JSON.stringify(paramObj)}, function(res){
        if (res.status == 200) {
          ...
          form.closest('.modal').hide();
          alert('Đã gửi yêu cầu báo giá thành công, chúng tôi liên hệ lại quý khách trong khoảng thời gian sớm nhất.');
        } else {
          alert('Có lỗi xảy ra, vui lòng thử lại.');
        }
      });
